...
_ENCRYPTION_MECHANISMS = {
    ...
    # TODO(dmend): Implement CMK_AES_GCM encrypt and decrypt
...
class PKCS11(object):
    def __init__(self, library_path, login_passphrase, rw_session, slot_id,
                 encryption_mechanism=None, hmac_keygen_mechanism=None,
    ...
    def compute_hmac(self, hmac_key, data, session):
        ...
        LOG.debug("alee: mechanism")
        LOG.debug(mech)
        if hmac_key is not None:
            LOG.debug("alee: hmac_key")
            LOG.debug(hmac_key)
